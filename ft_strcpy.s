...
copy_loop:
    mov al, [rdi]       ; charger le byte pointe par rsi dans al  (partie basse de rax )veux dire adresse pointeur
    mov [rsi], al       ; copier ce byte à l'adresse pointee par rdi
    test al, al         ; tester si al est nul (fin de chaîne)
    jz copy_end         ; si  c'est nul, sortir de la boucle avec le jum zro si flag = 0 
    inc rdi             ; avancer le pointeur source
    inc rsi             ; avancer le pointeur destination
